var UtilityFunctions;

UtilityFunctions = {
  currencyFormater: function(number) {
    var currencyCharacter, decimalcharacter, decimalplaces, dotpos, formatted, fraction, integer, sign, strnumber, temparray, thousandseparater;
    decimalplaces = 0;
    decimalcharacter = '';
    thousandseparater = ',';
    currencyCharacter = '$';
    number = parseFloat(number);
    sign = number < 0 ? '-' : '';
    formatted = new String(number.toFixed(decimalplaces));
    if (decimalcharacter.length && decimalcharacter !== '.') {
      formatted = formatted.replace(/\./, decimalcharacter);
    }
    integer = '';
    fraction = '';
    strnumber = new String(formatted);
    dotpos = decimalcharacter.length ? strnumber.indexOf(decimalcharacter) : -1;
    if (dotpos > -1) {
      if (dotpos) {
        integer = strnumber.substr(0, dotpos);
      }
      fraction = strnumber.substr(dotpos + 1);
    } else {
      integer = strnumber;
    }
    if (integer) {
      integer = String(Math.abs(integer));
    }
    while (fraction.length < decimalplaces) {
      fraction += '0';
    }
    temparray = new Array;
    while (integer.length > 3) {
      temparray.unshift(integer.substr(-3));
      integer = integer.substr(0, integer.length - 3);
    }
    temparray.unshift(integer);
    integer = temparray.join(thousandseparater);
    return sign + currencyCharacter + integer + decimalcharacter + fraction;
  }
};

module.exports = UtilityFunctions;
